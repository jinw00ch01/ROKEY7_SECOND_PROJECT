"""
OBB 학습 결과 시각화 (experiment_04).

experiment_03 대비 변경:
  - test_extra split이 별도 디렉토리이므로 per-class 메트릭과 sample predictions
    모두 일반 test와 unseen test_extra를 함께 시각화
  - confusion_matrix 복사 시 test_extra_* 도 포함
"""
...
def plot_training_curves(run_dir: Path, out_dir: Path):
    ...
    for ax, (tr, va, title) in zip(axes[0], loss_pairs):
        ...
        if va in df:
            ax.plot(df["epoch"], df[va], label="val", linewidth=2, linestyle="--")
...
def plot_per_class_metrics(model, data_yaml: Path, split: str, out_dir: Path, device, label: str | None = None):
    metrics = model.val(task="obb", data=str(data_yaml), split=split,
                        plots=False, verbose=False, device=device)
    p = np.asarray(metrics.box.p)
    r = np.asarray(metrics.box.r)
    map50 = np.asarray(metrics.box.ap50)
    map5095 = np.asarray(metrics.box.maps)
    if not len(p):
        print(f"[skip] no per-class metrics for {split}"); return
    label = label or split
    # ultralytics가 absent 클래스에 대해 0을 채우지 않을 수 있어 길이 맞춤
    nclass = len(NAMES)
    def pad(arr):
        a = np.zeros(nclass)
        a[:len(arr)] = arr[:nclass]
        return a
    p, r, map50, map5095 = pad(p), pad(r), pad(map50), pad(map5095)
    x = np.arange(nclass); width = 0.2
    ...
    ax.set_title(f"Per-class metrics on {label} set (OBB)")
    ...
    fig.tight_layout()
    out = out_dir / f"per_class_metrics_{label}.png"
    fig.savefig(out, dpi=120); plt.close(fig); print(f"[ok] {out}")
...
def plot_sample_predictions(model, out_dir: Path, device, n=8):
    test_img_dir = ROOT / "test" / "images"
    test_extra_img_dir = ROOT / "test_extra" / "images"
    extra_imgs = sorted(test_extra_img_dir.glob("*.jpg")) if test_extra_img_dir.exists() else []
    regular_imgs = sorted(test_img_dir.glob("*.jpg")) if test_img_dir.exists() else []
    extra = extra_imgs[: n // 2]
    regular = regular_imgs[: n - len(extra)]
    imgs = list(zip(extra, ["extra"] * len(extra))) + list(zip(regular, ["test"] * len(regular)))
    if not imgs:
        print("[skip] no test images found"); return

    cols = 4; rows = (len(imgs) + cols - 1) // cols
    fig, axes = plt.subplots(rows, cols, figsize=(cols * 4, rows * 3))
    axes = np.atleast_2d(axes)
    for ax, (img_path, kind) in zip(axes.flat, imgs):
...
def main():
    ...
    for prefix in ("train_phase1_", "train_phase2_"):
        try:
            plot_training_curves(latest_run(prefix), out_dir)
        except FileNotFoundError:
            pass
    ...
    if not weights.exists():
        print(f"[warn] no weights in {run_dir / 'weights'}; skipping live evals")
        return
    ...
    try:
        ...
        print("[warn] ultralytics not installed; skipping live evals")
        return
    ...
    if data_extra_yaml.exists():
        plot_per_class_metrics(model, data_extra_yaml, "test", out_dir, device, label="test_extra")

    val_dirs = []
    for prefix in ("val_", "test_", "test_extra_", "test_tuned_", "test_extra_tuned_"):
        try:
            val_dirs.append(latest_run(prefix))
        except FileNotFoundError:
            pass
    copy_confusion_matrices(val_dirs, out_dir)

    plot_sample_predictions(model, out_dir, device)
