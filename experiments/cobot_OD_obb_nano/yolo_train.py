"""
cobot_object_detection_obb_03 - YOLOv8s-OBB 2단계 학습 (experiment_03).

experiment_02 대비 변경:
  - task='obb'  (ultralytics가 라벨을 OBB로 해석)
  - 모델: yolov8s-obb.pt
  - 데이터: 24개 소스 통합 (cashew_extra 포함)
  - 평가 split: test는 (a) 기존 23셋 도메인 + (b) cashew_extra 미경험 세션 혼합
  - 회전 증강(degrees) 폭 확대 — OBB는 회전 불변성 학습이 본질

  성능 목표:
    - test mAP@0.5 (OBB)       >= 0.97
    - test mAP@0.5:0.95 (OBB)  >= 0.78
    - test cashew recall       >= 0.95
    - extra_cashew (unseen session) 부분 mAP@0.5 >= 0.90 → 일반화 검증
"""
...
def train_phase1(weights, data_yaml, ts, device):
    ...
    model.train(
        ...
        hsv_s=0.7,
        hsv_v=0.4,
        degrees=180.0,    # OBB는 임의 회전 견고성이 핵심 (axis-aligned는 10°였음)
        translate=0.1,
        scale=0.5,
        shear=0.0,
        perspective=0.0,
        flipud=0.5,       # OBB는 상하 반전도 안전 (회전 동치)
        fliplr=0.5,
        mosaic=1.0,
        mixup=0.1,
        copy_paste=0.0,   # OBB에서 copy_paste는 ultralytics가 미지원/불안정
        close_mosaic=10,
        multi_scale=0.5,
        cache=True,
        amp=True,
...
def train_phase2(phase1_dir: Path, data_yaml, ts, device):
    ...
    model.train(
        ...
        degrees=180.0,    # 회전 증강은 phase2에서도 유지 (OBB의 핵심)
...
def evaluate(run_dir: Path, data_yaml, ts, device, tta: bool):
    ...
    return ROOT / val_name, ROOT / test_name
...
def main():
    ...
    if args.phase in (1, 12):
        phase1_dir = train_phase1(initial, data_yaml, ts, device)
    if args.phase in (2, 12):
        if args.phase == 2:
            if not args.resume:
                raise SystemExit("--resume <phase1_dir> required when --phase 2")
            phase1_dir = ROOT / args.resume
        phase2_dir = train_phase2(phase1_dir, data_yaml, ts, device)
        target = phase2_dir
    else:
        target = phase1_dir

    val_dir, test_dir = evaluate(target, data_yaml, ts, device, tta=not args.no_tta)

    print("\n=== Done ===")
    if phase1_dir: print(f"phase1   : {phase1_dir}")
    if args.phase in (2, 12): print(f"phase2   : {target}")
    print(f"val run  : {val_dir}")
    print(f"test run : {test_dir}")
    print(f"best.pt  : {target / 'weights' / 'best.pt'}")
    print("\nNext: python inference_tune.py")
