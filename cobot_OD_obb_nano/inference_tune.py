"""
OBB 학습 후 conf/iou threshold 스윕 (experiment_03).

OBB도 동일하게 NMS는 conf+iou로 동작. 추가로 cashew_extra 분리 평가:
  - test split 안에는 (a) 일반 23셋 도메인 + (b) cashew_extra 미경험 세션이 섞여 있음
  - 두 그룹을 prefix로 분리해 각각의 P/R/mAP를 별도로 보고 → 도메인 갭 정량화
"""
...
def main():
    ...
    print("\nRe-evaluating test split with the best (conf, iou)...")
    tm = model.val(
